...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    if (major == 0 && minor == 0)
    {
        ...
        sscanf(gl_version_string, "%d.%d", &major, &minor);
    }
    bd->GlVersion = (GLuint)(major * 100 + minor * 10);

    const char* glsl_version_to_use = (glsl_version != nullptr && strncmp(glsl_version, "#version", 8) == 0) ? glsl_version : "#version 130";
    strncpy(bd->GlslVersionString, glsl_version_to_use, sizeof(bd->GlslVersionString) - 1);
    bd->GlslVersionString[sizeof(bd->GlslVersionString) - 1] = '\0';
