// ./GameFPS/VoxelGlobe/include/imgui/backends/imgui_impl_opengl3.cpp
...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    // GLEW is initialized in main.cpp, so we verify the context
    GLenum err = glGetError();
    if (err != GL_NO_ERROR)
    {
        fprintf(stderr, "OpenGL error %d before ImGui init!\n", err);
        return false;
    }
    ...
    // Query GL version (e.g., 320 for GL 3.2)
    GLint major = 0;
    GLint minor = 0;
    ...
    printf("GL_MAJOR_VERSION = %d\nGL_MINOR_VERSION = %d\nGL_VENDOR = '%s'\nGL_RENDERER = '%s'\n",
           major, minor, (const char*)glGetString(GL_VENDOR), (const char*)glGetString(GL_RENDERER));
#endif
    ...
    if (glsl_version == nullptr)
        glsl_version = "#version 330"; // Match Renderer.cpp shaders
    IM_ASSERT((int)strlen(glsl_version) + 2 < IM_ARRAYSIZE(bd->GlslVersionString));
    strcpy(bd->GlslVersionString, glsl_version);
    strcat(bd->GlslVersionString, "\n");

    // Make an arbitrary GL call to ensure context is active
